...
│
├── src
│   ├── GeneratedApp.Blazor      // MudBlazor UI only
...
8. For each selected table, generate:
   - Razor page using MudBlazor DataGrid/Table
...
17. Generate Program.cs with MudBlazor, EF Core, and endpoint registration.
...
20. Prefer a web-based scaffolder UI using ASP.NET Core + MudBlazor.
...
- When generating MudBlazor dialog forms:
- When scaffolding MudBlazor dialog forms: use MaxWidth.Small FullWidth, lay all fields out in a MudGrid Spacing="2" with 2 columns (xs="6"), and convert PascalCase field names to space-separated labels. Do not emit Required="false".
- Post-scaffold, a human should add section headers and promote long fields to xs="12".
